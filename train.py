...
def main(params):
    ...
    if params.category_sampler:
        ...
        all_targets = []
        temp_dataloader = DataLoader(dataset(params.root, if_train=True), batch_size=params.batch_size,num_workers=4,
                                shuffle=True)
        ...
        dataloaders = {
            'train': DataLoader(dataset(params.root, if_train=True), batch_size=params.batch_size, num_workers=4,
                                shuffle=False, sampler=category_sampler),
            'test': DataLoader(dataset(params.root, if_train=False), batch_size=params.batch_size, num_workers=4,
                            shuffle=False),
    ...
    if params.eval_only is False:
        ...
        for epoch in range(params.epochs):
            ...
            if epoch % params.eval_epoch == 0 or epoch == params.epochs - 1:
                ...
                mAP_n = compute_mAP_n(train_binary, test_binary, train_label, test_label)
